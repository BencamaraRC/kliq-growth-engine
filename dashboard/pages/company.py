"""Company — KLIQ revenue overview with MRR, fees breakdown, trend, and per-coach table."""

import streamlit as st

st.set_page_config(page_title="Company | KLIQ", layout="wide")
...
st.title("Company")
st.caption("KLIQ revenue — platform fees (% of GMV) + coach hosting subscriptions")
...
# Show warning if all values are zero (likely CMS DB not connected)
if mrr == 0 and kliq_data["gmv"] == 0 and kliq_data["active_coaches"] == 0:
    st.warning("CMS database not reachable — revenue data will show $0 until connected.")
