...
def render_store_preview(
    ...
    if blogs:
        ...
        pinned_img = (
            f'<img src="{pinned_thumb_b64}" style="width:100%;height:160px;border-radius:10px;object-fit:cover;display:block;margin:10px 0;" />'
            if pinned_thumb_b64
            else ""
        )
...
<div style="display:flex;flex-direction:column;align-items:center;margin:0 auto;padding:16px 0;">
    ...
    <div style="display:flex;width:393px;flex-direction:column;align-items:flex-start;background:{
        card_bg
    };border-radius:20px;box-shadow:{shadow_md};overflow:hidden;position:relative;">

        {revenue_notifications_html}

        <!-- NAV BAR -->
        <div style="display:flex;align-items:center;justify-content:space-between;width:100%;padding:12px 16px;border-bottom:1px solid {
        border_color
        ...
        text_primary
    };white-space:nowrap;">{store_name[:20]}</span>
            </div>
            <div style="display:flex;gap:6px;">
                <span style="border:1px solid {
        border_color
    };border-radius:8px;padding:5px 12px;font-size:11px;color:{
        text_primary
    };font-weight:500;cursor:pointer;">Log in</span>
                <span style="background:{
        kliq_green
    };border-radius:8px;padding:5px 12px;font-size:11px;color:#fff;font-weight:600;cursor:pointer;">Sign up</span>
            </div>
        </div>
        ...
        <div style="display:flex;width:100%;padding:16px;box-sizing:border-box;align-items:flex-start;gap:12px;">
            ...
            <div style="display:flex;padding:0 16px;flex-direction:column;justify-content:center;align-items:flex-start;gap:16px;flex:1 0 0;min-width:0;">
                <div>
                    <h2 style="color:{
        text_primary
    };font-weight:600;font-size:16px;margin:0;line-height:120%;letter-spacing:-0.02em;">{
        store_name
    }</h2>
                    {
        ""
        if not niche_subtitle
        else f'<p style="color:{text_tertiary};font-size:12px;margin:4px 0 0;line-height:140%;">{niche_subtitle}</p>'
    }
                </div>
        ...
        <div style="width:100%;padding:0 16px 16px;box-sizing:border-box;">
            <p style="color:{text_secondary};font-size:12px;margin:0 0 12px;line-height:140%;">{
        short_bio[:200]
    }</p>
            <div style="display:flex;gap:8px;">
                <span style="background:{
        kliq_green
    };color:#fff;border-radius:8px;padding:8px 20px;font-size:12px;font-weight:600;cursor:pointer;">Sign up</span>
                <span style="border:1px solid {
        border_color
    };border-radius:8px;padding:8px 20px;font-size:12px;font-weight:500;color:{
        text_primary
        ...
        text_primary
                        ...
                        <h3 style="font-weight:600;font-size:13px;color:{
        text_primary
    };letter-spacing:-0.02em;">Ask me anything</h3>
                ...
                <p style="color:{
        text_tertiary
    };font-size:11px;margin:0 0 10px;line-height:140%;">I'm here for whatever expert advice you need.</p>
                {ama_chat_html}
            </div>
            ...
            <div>
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
                    <h3 style="font-weight:600;font-size:14px;color:{
        text_primary
    };letter-spacing:-0.02em;">Live streams</h3>
                    <span style="font-size:11px;color:{
        text_secondary
    };cursor:pointer;border:1px solid {
        border_color
    };border-radius:8px;padding:3px 10px;">See all</span>
                </div>
        ...
        if not products
            ...
            <div>
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
                    <h3 style="font-weight:600;font-size:14px;color:{
        text_primary
    };letter-spacing:-0.02em;">Featured</h3>
                    <span style="font-size:11px;color:{
        text_secondary
    };cursor:pointer;border:1px solid {
        border_color
                ...
                <div style="display:flex;gap:10px;overflow-x:auto;padding-bottom:4px;">
                    <div style="display:flex;width:320px;padding:12px;flex-direction:column;align-items:flex-start;gap:16px;flex-shrink:0;border-radius:8px;border:1px solid #F3F4F6;background:#fff;">
                        <div style="width:100%;height:140px;border-radius:8px;background:linear-gradient(135deg,{
        kliq_green
    } 0%,{tangerine} 100%);display:flex;align-items:center;justify-content:center;">
        ...
        text_primary
    };line-height:120%;">Getting Started Guide</span>
                            <span style="font-size:11px;color:{
        text_tertiary
    };line-height:140%;">Everything you need to begin your transformation journey</span>
                        </div>
                        <span style="background:{
        kliq_green
    };color:#fff;border-radius:8px;padding:8px 20px;font-size:12px;font-weight:600;cursor:pointer;">Watch now</span>
                    </div>
                    <div style="display:flex;width:320px;padding:12px;flex-direction:column;align-items:flex-start;gap:16px;flex-shrink:0;border-radius:8px;border:1px solid #F3F4F6;background:#fff;">
                        <div style="width:100%;height:140px;border-radius:8px;background:linear-gradient(135deg,#DEFE9C 0%,{
        kliq_green
                        ...
                        <div style="display:flex;flex-direction:column;gap:4px;">
                            <span style="font-weight:600;font-size:13px;color:{
        text_primary
    };line-height:120%;">Nutrition Blueprint</span>
                            <span style="font-size:11px;color:{
        text_tertiary
    };line-height:140%;">Meal plans, macros, and recipes tailored for your goals</span>
                        </div>
                        <span style="background:{
        kliq_green
    };color:#fff;border-radius:8px;padding:8px 20px;font-size:12px;font-weight:600;cursor:pointer;">Download</span>
                    </div>
                </div>
            </div>
            ...
            <div style="display:flex;width:361px;padding:24px 0;flex-direction:column;align-items:center;gap:12px;box-sizing:border-box;">
                <div style="width:100%;border-radius:8px;border:1px solid #F3F4F6;background:#fff;padding:16px;">
                    <h3 style="font-weight:600;font-size:14px;color:{
        text_primary
    };letter-spacing:-0.02em;margin:0 0 8px;">About</h3>
                    <p style="font-size:12px;color:{text_secondary};line-height:140%;margin:0;">{
        short_bio[:300]
        if short_bio
        else "Passionate coach helping you achieve your fitness and wellness goals through personalised programs and expert guidance."
    }</p>
                </div>
                <div style="display:flex;gap:10px;width:100%;">
                    <div style="display:flex;padding:12px;flex-direction:column;align-items:flex-start;gap:16px;flex:1 0 0;border-radius:8px;border:1px solid #F3F4F6;background:#fff;">
                        <span style="font-weight:600;font-size:20px;color:{
        text_primary
    };line-height:120%;letter-spacing:-0.02em;">500+</span>
                        <span style="font-size:11px;color:{
        text_tertiary
    };line-height:140%;">Members</span>
                    </div>
                    <div style="display:flex;padding:12px;flex-direction:column;align-items:flex-start;gap:16px;flex:1 0 0;border-radius:8px;border:1px solid #F3F4F6;background:#fff;">
                        <span style="font-weight:600;font-size:20px;color:{
        text_primary
                        ...
                        <span style="font-size:11px;color:{
        text_tertiary
    };line-height:140%;">Programs</span>
                    </div>
                    <div style="display:flex;padding:12px;flex-direction:column;align-items:flex-start;gap:16px;flex:1 0 0;border-radius:8px;border:1px solid #F3F4F6;background:#fff;">
                        <span style="font-weight:600;font-size:20px;color:{
        text_primary
    };line-height:120%;letter-spacing:-0.02em;">4.9</span>
                        <span style="font-size:11px;color:{
        text_tertiary
    };line-height:140%;">Rating</span>
                    </div>
                </div>
            </div>
            ...
            <div>
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
                    <h3 style="font-weight:600;font-size:14px;color:{
        text_primary
    };letter-spacing:-0.02em;">Community</h3>
                    <span style="font-size:11px;color:{
        text_secondary
    };cursor:pointer;border:1px solid {
        border_color
                ...
                <div style="display:flex;flex-direction:column;gap:10px;">
                    <div style="display:flex;padding:12px;flex-direction:column;align-items:flex-start;gap:16px;border-radius:8px;border:1px solid #F3F4F6;background:#fff;">
                        <div style="display:flex;align-items:center;gap:8px;">
                            <div style="width:28px;height:28px;border-radius:50%;background:#FFECE7;display:flex;align-items:center;justify-content:center;flex-shrink:0;"><span style="font-size:11px;font-weight:600;color:{
        tangerine
    };">S</span></div>
                            <div>
                                <span style="font-weight:600;font-size:12px;color:{
        text_primary
    };">Sarah M.</span>
                                <div style="display:flex;gap:1px;">
                                    <svg width="10" height="10" viewBox="0 0 24 24" fill="{
        tangerine
    }" stroke="none"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                                    <svg width="10" height="10" viewBox="0 0 24 24" fill="{
        tangerine
    }" stroke="none"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                                    <svg width="10" height="10" viewBox="0 0 24 24" fill="{
        ...
        text_secondary
    };line-height:140%;margin:0;">"This program completely changed my routine. I'm stronger, more confident, and actually enjoy working out now."</p>
                    </div>
                    <div style="display:flex;padding:12px;flex-direction:column;align-items:flex-start;gap:16px;border-radius:8px;border:1px solid #F3F4F6;background:#fff;">
                        <div style="display:flex;align-items:center;gap:8px;">
        ...
        tangerine
                        ...
                        <p style="font-size:12px;color:{
        text_secondary
    };line-height:140%;margin:0;">"The nutrition plans alone are worth it. Lost 8kg in 3 months and never felt deprived."</p>
                    </div>
                </div>
            </div>
        ...
        text_tertiary
                ...
                <svg width="16" height="16" viewBox="0 0 24 24" fill="{
        text_tertiary
    }"><path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zM9 16V8l8 3.993L9 16z"/></svg>
            </div>
            <p style="font-weight:400;color:{
        text_tertiary
    };font-size:11px;">Powered by <span style="font-weight:600;color:{kliq_green};">KLIQ</span></p>
        </div>
